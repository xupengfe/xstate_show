...
/* Bits 0-1 must be 0 for RW access to linear address of the process itself. */
#define POPULATE_PKRU_DATA	0xfffffffc
...
static void check_cpuid_xstate(void)
{
...
static void fill_xstate_buf(uint8_t data, unsigned char *buf, int xfeature_num)
{
	...
	for (i = 0; i < xstate_info.size[xfeature_num]; i++)
		buf[xstate_info.offset[xfeature_num] + i] = data;
}
...
{
	uint32_t i, pkru_data = POPULATE_PKRU_DATA;
	...
	if (xstate_tested(XFEATURE_PKRU)) {
		/* Only 0-3 bytes pkru xstates are allowed to be written. */
		memcpy((unsigned char *)buf + xstate_info.offset[XFEATURE_PKRU],
			(void *)&pkru_data, sizeof(uint32_t));
	}
}
...
static void test_xstate_sig_handle(void)
{
	 ...
	 */
	xrstor(valid_xbuf, xstate_info.mask);
	__raise(process_pid, SIGUSR1);
	xsave(compared_xbuf, xstate_info.mask);
