...
/* Test XMM and -mno-sse when compile, no stdlib.h and need declaration. */
...
static void fill_xstates_buf(struct xsave_buffer *buf, uint32_t xsave_mask)
{
	...
	if (xstate_info.xstate_flag[XFEATURE_YMM] == SUPPORT)
		fill_xstate_buf(byte_data, (unsigned char *)buf, XFEATURE_YMM);

	/*
	 * Fill specific byte data value into AVX512 OPMASK/ZMM xstates buffer
	 * (AVX512 OPMASK/ZMM offset/size).
	 */
	if (xstate_info.xstate_flag[XFEATURE_OPMASK] == SUPPORT)
		fill_xstate_buf(byte_data, (unsigned char *)buf, XFEATURE_OPMASK);
	if (xstate_info.xstate_flag[XFEATURE_ZMM_Hi256] == SUPPORT)
		fill_xstate_buf(byte_data, (unsigned char *)buf, XFEATURE_ZMM_Hi256);
	if (xstate_info.xstate_flag[XFEATURE_Hi16_ZMM] == SUPPORT)
		fill_xstate_buf(byte_data, (unsigned char *)buf, XFEATURE_Hi16_ZMM);
